...
class Conf:
    ...
    def __init__(
        ...
        if pkt_list is not None:
            ...
            self.pkt_list: list[Packet] | None  = pkt_list
            """ 被处理的恶意包列表，用于构建向量与重构流量   \n 默认值：None """
            self.last_end_time: EDecimal | None = last_end_time
            """ 上一条流结束的时间   \n 默认值：None"""
        self.max_cft_pkt: int          = max_cft_pkt
        """ 每个原始包对应构建包的最大数量(l_c)   \n 默认值：1 """
        self.max_cft_pkt_prob: float   = max_cft_pkt_prob
        """ 在 0-1 之间的概率,限制一个slot填入构造包的最大概率,相当于 max_cft_pkt 的最大乘数   \n 默认值：0.01 """
        self.max_time_extend: float    = max_time_extend
        """ 新流量相较于原始的时间倍数(l_t)   \n 默认值 6.0 """
        self.min_time_extend: float    = min_time_extend
        """ 新流量相较于原始的时间倍数(l_t)   \n 默认值 3.0 """
        self.fence_time_divider: int   = fence_time_divider
        """ 用于计算原始包之间的最小时间间隔,代表最大允许时间间隔中存在多少个可用时间位置   \n 默认值：10000 """
        self.cft_time_divider: int     = cft_time_divider
        """ 用于计算构建包与前个包之间的最小时间间隔,代表在构建包与前个包时间间隔中存在多少个可用时间位置   \n 默认值 1000 """
        self.proto_min_lmt: float      = proto_min_lmt
        """ 用于限制构建包最小使用的协议层数   \n 默认值：1.0"""
        self.data_max_lmt: list[float] = data_max_lmt
        """ 用于限制构建包的 mtu, list 的序号对应协议层数, 如序号 2 限制 2 层包的 mtu    \n 默认值：[np.nan, 1500., 1480., 1460.]"""
        self.data_min_lmt: float       = data_min_lmt
